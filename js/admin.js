...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 🌟 เพิ่มคำสั่งโหลดกราฟตรงนี้
    loadAnalytics();

    if (document.getElementById('add-camp-form')) document.getElementById('add-camp-form').addEventListener('submit', handleAddCamp);
...
async function fetchClearances(status) {
    ...
    try {
            let query = supabaseClient.from('clearances').select(`id, status, request_type, purpose, department, total_amount, actual_amount, camp_id, created_at, profiles!user_id(full_name), camps(name)`).order('created_at', { ascending: false });        if (status === 'pending_expense') {
    ...
    } catch (err) { container.innerHTML = `<p class="text-red-500 text-center">เกิดข้อผิดพลาด: ${err.message}</p>`; }
}
...
function renderClearances(clearances) {
    ...
    clearances.forEach(item => {
        ...
        const safePurpose = item.purpose.replace(/'/g, "\\'"); // ป้องกัน error ถ้าชื่อมีเครื่องหมายคำพูด
        ...
        if (item.status === 'pending') {
            ...
            actionButtonsHTML = `
                <button onclick="openApproveModal('${item.id}', ${item.total_amount}, '${item.camp_id}', '${item.request_type}')" class="flex-1 bg-gradient-to-r ${isInc ? 'from-pink-500 to-rose-500':'from-green-500 to-emerald-500'} text-white text-xs font-bold py-2.5 rounded-xl transition-transform hover:scale-[1.02] flex justify-center items-center gap-1"><i data-lucide="check" class="w-4 h-4"></i> ${btnText}</button>
        ...
        // 🌟 จุดที่เพิ่มเข้ามาใหม่: สำหรับรายการที่โอนเงินทดรองไปแล้วแต่รอเคลียร์ หรือถูกตีกลับให้แก้บิลเคลียร์
            ...
            actionButtonsHTML = `<div class="flex-1 text-center py-2.5 text-xs font-bold text-gray-500 bg-gray-50 rounded-xl border border-gray-200">ดำเนินการเรียบร้อยแล้ว</div>`;
        }
...
async function updateStatus(clearanceId, newStatus, purpose = 'รายการนี้') {
    // 🌟 เพิ่ม Pop-up ยืนยันการตีกลับที่มีรายละเอียด
...
async function openApproveModal(clearanceId, amount, campId, mode, actualAmount = 0) {
    ...
    let titleText = 'ยอดเงิน';
    const isIncome = mode === 'income' || mode === 'other_income';
    const diff = amount - actualAmount; 

    if (isIncome) {
        titleText = 'ยอดเงินที่ต้องนำเข้าบัญชีกลาง';
    } else if (mode === 'reconcile') {
        if (diff > 0) titleText = 'ยอดเงินทอนที่ต้องตรวจสอบและรับคืน';
        else if (diff < 0) titleText = 'ยอดเงินชดเชยที่แอดมินต้องโอนเพิ่ม';
        displayAmount = Math.abs(diff);
    } else {
        titleText = 'ยอดเงินที่เหรัญญิกต้องสแกนจ่ายออก';
    }
    ...
    bankSelect.innerHTML = '<option>กำลังโหลดบัญชี...</option>';
    fundSelect.innerHTML = '<option>กำลังโหลดงบ...</option>';
    // 👇 โค้ดที่ต้องเพิ่ม: ดึงข้อมูลบัญชีมารอไว้
...
if (!isIncome && cInfo && cInfo.receive_bank_account) {
    document.getElementById('approve-amount-display').parentNode.insertAdjacentHTML('afterend', `
        <div id="approve-receive-bank-info" class="bg-blue-50 p-3 rounded-xl border border-blue-100 mt-3 text-left shadow-sm">
            <p class="text-blue-800 font-bold text-xs mb-1"><i data-lucide="arrow-down-right" class="w-3.5 h-3.5 inline"></i> ต้องโอนเงินไปที่บัญชีนี้:</p>
            ...
            <p class="text-blue-700 text-xs mt-0.5">ชื่อบัญชี: ${cInfo.receive_account_name || 'ไม่ระบุชื่อ'}</p>
        </div>
    `);
}
// 👆 จบโค้ดที่เพิ่ม
    ...
    try {
        ...
        const { data: funds } = await supabaseClient.from('funds').select('*'); // 👈 ดึงกองทุนทั้งหมดเลย ไม่ต้องฟิลเตอร์ค่าย
...
// 🌟 เพิ่มตัวแปรล็อก ป้องกันแอดมินกดย้ำ/คลิกเบิ้ล
...
async function confirmApproval() {
    ...
    if (!bankSelect.value || !fundSelect.value) return Swal.fire('ข้อมูลไม่ครบ', 'กรุณาเลือกบัญชีและกองทุนให้ครบถ้วน', 'warning');
    ...
    if (isExpenseMode) {
        if (bankBalance < requiredAmount) {
            return Swal.fire('❌ ยอดเงินบัญชีกลางไม่พอ', `บัญชี ${bankName} มีเงินเหลือ <b>${bankBalance.toLocaleString('th-TH')} ฿</b><br>แต่ต้องใช้โอนออก <b>${requiredAmount.toLocaleString('th-TH')} ฿</b>`, 'error');
        }
        if (fundBalance < requiredAmount) {
            return Swal.fire('❌ ยอดเงินกองทุนไม่พอ', `กองทุน ${fundName} มีเงินเหลือ <b>${fundBalance.toLocaleString('th-TH')} ฿</b><br>แต่ต้องใช้ตัดยอด <b>${requiredAmount.toLocaleString('th-TH')} ฿</b>`, 'error');
        }
    }
    ...
    if (isIncomeMode) {
        actionTitle = 'ยืนยันรับเงินเข้าบัญชี?';
        warningHTML = `<ul class="list-disc ml-5 space-y-1"><li>ยอดเงินจะถูก <b class="text-green-600">เพิ่มเข้าบัญชีกลางอัตโนมัติ</b></li><li>ระบบจะบันทึกสถานะเป็น <b>เสร็จสิ้น (Cleared)</b></li></ul>`;
    } else if (mode === 'reconcile') {
        displayAmount = Math.abs(diff);
        if (diff > 0) {
            actionTitle = 'ยืนยันรับเงินทอนคืน?';
            warningHTML = `<ul class="list-disc ml-5 space-y-1"><li>ยอดเงินส่วนต่างจะถูก <b class="text-green-600">เพิ่มเข้าบัญชีกลางอัตโนมัติ</b></li><li>ระบบจะตัดสถานะเป็น <b>เคลียร์บิลเรียบร้อย</b></li></ul>`;
        } else if (diff < 0) {
            actionTitle = 'ยืนยันโอนเงินชดเชย?';
            warningHTML = `<ul class="list-disc ml-5 space-y-1"><li>ยอดเงินชดเชยจะถูก <b class="text-red-600">หักออกจากบัญชีกลางอัตโนมัติ</b></li><li>โปรดตรวจสอบว่าแนบสลิปถูกต้องก่อนกดยืนยัน</li></ul>`;
        } else {
            actionTitle = 'ยืนยันปิดบิล (ยอดพอดี)?';
            warningHTML = `<ul class="list-disc ml-5 space-y-1"><li>จะไม่มีการบวก/ลบเงินจากบัญชีกลาง</li><li>ระบบจะปิดยอดรายการนี้เป็น <b>เคลียร์บิลเรียบร้อย</b></li></ul>`;
        }
    } else {
        actionTitle = 'ยืนยันอนุมัติจ่ายเงิน?';
        warningHTML = `<ul class="list-disc ml-5 space-y-1"><li>ยอดเงินจะถูก <b class="text-red-600">หักออกจากบัญชีกลางอัตโนมัติ</b></li><li>สถานะคำขอจะเปลี่ยนเป็น <b>รอเคลียร์บิล / เสร็จสิ้น</b></li></ul>`;
    }
    ...
    const confirmResult = await Swal.fire({
        ...
        html: `
            <div class="text-left text-sm mt-4 border-t border-gray-100 pt-4">
                <div class="mb-3 space-y-1.5">
                    ...
                    <p class="text-gray-500">บัญชี: <span class="text-gray-800 font-bold">${bankName}</span></p>
                    <p class="text-gray-500">กองทุน: <span class="text-gray-800 font-bold">${fundName}</span></p>
                </div>
                <div class="bg-amber-50 p-3 rounded-xl border border-amber-200 mt-4">
                    <p class="text-amber-700 font-bold text-xs mb-1">⚠️ ผลของการดำเนินการนี้:</p>
        ...
        confirmButtonText: 'ใช่, ดำเนินการทันที',
        cancelButtonText: 'ยกเลิก',
        reverseButtons: true
    });
    ...
    btn.disabled = true; 
    btn.innerHTML = '<i data-lucide="loader-2" class="w-4 h-4 inline animate-spin"></i> กำลังบันทึกรายการ...';
    lucide.createIcons();
    ...
    try {
        ...
        if (!document.getElementById('admin-slip-upload-section').classList.contains('hidden')) {
            ...
            if (fileInput.files.length === 0) throw new Error('กรุณาแนบภาพสลิปหลักฐานการโอนเงินออกด้วยครับ');
        ...
        if (isIncomeMode) {
            await supabaseClient.from('transactions').insert([{ camp_id: campId, bank_account_id: bankSelect.value, fund_id: fundSelect.value, amount: amount, transaction_type: 'income', description: `อนุมัติรับเงินเข้าบัญชีกลาง`, created_by: session.user.id, clearance_id: clearanceId }]);
            await supabaseClient.from('clearances').update({ status: 'cleared' }).eq('id', clearanceId);

        } else if (mode === 'reimburse' || mode === 'other_expense') {
            await supabaseClient.from('transactions').insert([{ camp_id: campId, bank_account_id: bankSelect.value, fund_id: fundSelect.value, amount: amount, transaction_type: 'expense', description: `อนุมัติปิดยอดจ่ายเงินเรียบร้อย`, created_by: session.user.id, clearance_id: clearanceId }]);
            await supabaseClient.from('clearances').update({ status: 'cleared', advance_slip_url: uploadedSlipUrl, actual_amount: amount }).eq('id', clearanceId);

        } else if (mode === 'advance') {
            await supabaseClient.from('transactions').insert([{ camp_id: campId, bank_account_id: bankSelect.value, fund_id: fundSelect.value, amount: amount, transaction_type: 'expense', description: `อนุมัติโอนเงินยืมทดรองจ่าย`, created_by: session.user.id, clearance_id: clearanceId }]);
            await supabaseClient.from('clearances').update({ status: 'approved', advance_slip_url: uploadedSlipUrl }).eq('id', clearanceId);

        } else if (mode === 'reconcile') {
            let payload = { status: 'cleared' };
            if (diff > 0) {
                await supabaseClient.from('transactions').insert([{ camp_id: campId, bank_account_id: bankSelect.value, fund_id: fundSelect.value, amount: diff, transaction_type: 'income', description: `รับเงินทอนคืนจากการเคลียร์บิล`, created_by: session.user.id, clearance_id: clearanceId }]);
            } else if (diff < 0) {
                const absDiff = Math.abs(diff);
                await supabaseClient.from('transactions').insert([{ camp_id: campId, bank_account_id: bankSelect.value, fund_id: fundSelect.value, amount: absDiff, transaction_type: 'expense', description: `โอนเงินชดเชยส่วนต่างค่ายเคลียร์บิล`, created_by: session.user.id, clearance_id: clearanceId }]);
                payload.reimburse_slip_url = uploadedSlipUrl;
            }
            await supabaseClient.from('clearances').update(payload).eq('id', clearanceId);
        }

        await Swal.fire('สำเร็จ', 'อนุมัติรายการเรียบร้อย! (ยอดเงินอัปเดตอัตโนมัติ)', 'success');
        closeApproveModal(); 
        fetchClearances(currentTab); 
        updateBadges();
...
async function viewDetails(clearanceId) {
    ...
    try {
        // 1. แก้ไข Query ให้ระบุ profiles!user_id ให้ชัดเจน
        ...
        // 2. ดึงชื่อให้ถูกต้อง (ถ้าระบุบุคคลภายนอก ให้ใช้ unregistered_name ถ้าเป็นสมาชิกให้ดึง full_name)
        // ดึงชื่อให้ถูกต้อง
        const applicantName = clearance.unregistered_name || clearance.profiles?.full_name || 'ไม่ระบุชื่อ';

        // 👇 โค้ดที่ต้องเพิ่ม: สร้างกล่องบัญชีผู้รับเงิน
        let receiveBankHTML = '';
        ...
        content.innerHTML = `
            <div class="space-y-2">
                <p><b>ผู้ขอ:</b> ${applicantName}</p>
                <p><b>โครงการ:</b> ${clearance.camps?.name || 'ไม่ระบุ'}</p>
                <p><b>วัตถุประสงค์:</b> ${clearance.purpose}</p>
                ${receiveBankHTML}
                <div class="bg-gray-50 p-3 rounded-xl mt-3">
                    <p class="font-bold border-b pb-1 mb-2">รายการเบิก</p>
                    ${itemsHTML || '<p class="text-gray-400">ไม่มีข้อมูลรายการ</p>'}
                </div>
                <div class="pt-2 mt-4 border-t border-gray-100">
                    <p class="text-xs font-bold text-gray-700 mb-2 mt-2">หลักฐาน / ใบเสร็จ</p>
                    ${evidenceHTML || '<p class="text-xs text-gray-400">ไม่มีไฟล์แนบ</p>'}
                </div>
            </div>`;
...
async function deleteCamp(campId, campName) {
    // 🌟 เพิ่ม Pop-up ยืนยันการลบค่ายแบบละเอียดและเตือนให้ชัดเจน
...
async function setActiveCamp(targetId, campName) {
    // 🌟 เพิ่ม Pop-up ยืนยันการสลับค่ายปัจจุบัน
    const result = await Swal.fire({
...
// 🌟 ฟังก์ชันกราฟสรุปภาพรวม (เวอร์ชันกราฟเส้น - กระแสเงินสดตามวัน)
async function loadAnalytics() {
    try {
        const { data: camp } = await supabaseClient.from('camps').select('id').eq('is_active', true).single();
        if (!camp) return;

        // 🌟 ดึงข้อมูลวันที่ทำรายการ (created_at) มาด้วย
        ...
        safeClearances.forEach(doc => {
            ...
            if (doc.request_type === 'income' || doc.request_type === 'other_income') {
                ...
                // แปลงเวลาให้เป็นวันที่ (YYYY-MM-DD) เพื่อจัดกลุ่ม
        ...
        // อัปเดต Progress Bar ใช้งบ/รายรับ
        ...
        if (ctx) {
            ...
            // 🌟 ตั้งค่าให้แสดงผลเป็น กราฟเส้น (Line Chart) สไตล์ Modern
            window.myDonutChart = new Chart(ctx, {
                type: 'line', // เปลี่ยนจาก doughnut เป็น line
                data: {
                    labels: chartLabels,
                    datasets: [{
                        label: 'ยอดใช้จ่าย (บาท)',
                        data: chartData,
                        borderColor: '#3b82f6', // สีน้ำเงินของเส้น
                        backgroundColor: 'rgba(59, 130, 246, 0.1)', // สีพื้นใตักราฟแบบโปร่งใส
                        borderWidth: 2,
                        ...
                        tension: 0.4 // ทำเส้นให้โค้งมนดูสวยงาม
                    }]
                },
                options: {
                    responsive: true, 
                    ...
                    plugins: {
                        legend: { display: false }, // ซ่อนป้ายกำกับหัวกราฟ (เพราะมีเส้นเดียว)
                        tooltip: { 
                            callbacks: { 
                    ...
                    scales: {
                        x: {
                            grid: { display: false } // ซ่อนเส้นตารางแนวตั้งให้มินิมอล
                        },
                        y: {
                            beginAtZero: true,
                            ticks: {
                                callback: function(value) {
                                    return value.toLocaleString('th-TH'); // ใส่คอมม่าให้ตัวเลขแกน Y
                                }
                            }
                        }
                    }
...
// 🌟 ฟังก์ชันออกจากระบบ (อัปเดตมี Pop-up ยืนยัน)
